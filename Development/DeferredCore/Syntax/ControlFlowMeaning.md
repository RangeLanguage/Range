...
This is bounded C compiler machinery, not executable Core control-flow definitions.
Core's Execution construct and retained syntax alone do not implement these
transfer/join rules. The compiler still needs the corresponding graph analysis.
This pass does not introduce a general CFG abstraction, implement macro execution,
or prove aggregate ownership/COW. Existing unsupported closures and binding inputs
remain unsupported. The binder still validates unsupported syntax in unreachable
code even though initialization analysis stops at terminating paths.

Evidence: Testing/Compiler/Pass/Initialization.range and the initialization failure
fixtures execute through Testing/Tools/check-compiler-evaluator, in ordinary and
