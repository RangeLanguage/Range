...
- `compilerBParseLocalBindingProcessFacet` retains the local RHS span and an
  expression relationship. This differs from the member path; it is a useful
  existing mechanism, not proof of the agreed resolution semantics.
- `compilerBParseFunctionParameterFacets` records keyword, identity, first type
  token, and ordinal. Input records still need the full RHS syntax relationship
  and materialization as Let/Binding members.
- `Language/Recovery/Interpreter/parser.c`, `atTypePosition`: every name token
...
- `Language/Recovery/Interpreter/evaluator.c`: bare RHS names now use ordinary
...
Preservation follow-up: canonical member tests execute the Range parser for bare
construct/member references, empty construct/function calls, nested arguments,
and an integer literal. Recovery structural tests assert exact source spans and
