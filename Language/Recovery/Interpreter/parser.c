...
static const char *parseTypeName(RangeParser *parser, int *flags, RangeNode **generics)
{
    *flags = 0;
    ...
    while (parserAt(parser, "|")) {
        if (*generics) {
            parserFail(parser, &parser->current, "specialized union types are not supported by the recovery parser");
            return name;
        }
        parserAdvance(parser);
...
/* @syntax declares surface grammar, not an expression. The recovery seed
